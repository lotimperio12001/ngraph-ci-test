...
RUN cmake -DNGRAPH_DEX_ONLY=TRUE DNGRAPH_TOOLS_ENABLE=FALSE -DNGRAPH_USE_PREBUILT_LLVM=TRUE -DNGRAPH_CPU_ENABLE=TRUE -DCMAKE_INSTALL_PREFIX=/root/ngraph_dist -DNGRAPH_ONNX_IMPORT_ENABLE=TRUE -DCMAKE_BUILD_TYPE=Release ..
RUN make -j"$(nproc)"
RUN make install
